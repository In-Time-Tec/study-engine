...
impl Db {
    pub fn open() -> Result<Self> {
        let path = if let Ok(env_path) = std::env::var("STUDY_ENGINE_DB_PATH") {
            std::path::PathBuf::from(env_path)
        } else {
            crate::paths::home_dir().join(".local/share/study-engine/study-engine.db")
        };
        Self::open_at(&path)
    }
    ...
    pub fn get_card(&self, user_key: &str, cert: &str, id: &str) -> Result<CardState> {
        tracing::trace!(card_id = %id, %cert, %user_key, "get_card");
    ...
    pub fn insert_session(&self, user_key: &str, cert: &str, total: u32, correct: u32) -> Result<()> {
        let date = Local::now().format("%Y-%m-%d").to_string();
        tracing::debug!(%cert, %user_key, total, correct, "insert_session");
        self.0.execute(
            "INSERT INTO sessions (user_key, cert, date, total, correct) VALUES (?1,?2,?3,?4,?5)",
            params![user_key, cert, date, total, correct],
        )?;
        Ok(())
    }
    ...
    pub fn record_review(
        ...
        tracing::debug!(%card_id, %cert, %user_key, rating, correct = correct as u8, reps = card.reps, due = ?card.due, "record_review");
        ...
        tx.execute(
            "INSERT INTO reviews (user_key, card_id, cert, ts, correct, rating, selected_letter)
             VALUES (?1,?2,?3,?4,?5,?6,?7)",
            params![user_key, card_id, cert, ts, correct as i32, rating, selected_letter],
        )?;
        tx.commit()?;
        Ok(())
    }
    ...
    pub fn reviews_since(
        ...
        let sql = format!(
            "SELECT card_id, correct, rating, selected_letter
             FROM reviews
             WHERE user_key = ?1 AND cert = ?2 AND ts >= ?3 AND card_id IN ({})
             ORDER BY ts",
            placeholders
        );
        let mut stmt = self.0.prepare(&sql)?;
        let mut raw_params: Vec<Box<dyn rusqlite::ToSql>> = vec![
            Box::new(user_key.to_string()),
    ...
    pub fn clear_pending_session(&self, user_key: &str, cert: &str) -> Result<()> {
        self.0.execute(
            "DELETE FROM pending_sessions WHERE user_key = ?1 AND cert = ?2",
            params![user_key, cert],
        )?;
        Ok(())
    }
    ...
    #[cfg(test)]
    pub fn reviews_for(&self, user_key: &str, cert: &str, card_id: &str) -> Result<Vec<(bool, u32)>> {
        let mut stmt = self.0.prepare(
            "SELECT correct, rating FROM reviews WHERE user_key=?1 AND cert=?2 AND card_id=?3 ORDER BY ts",
        )?;
        let rows = stmt.query_map(params![user_key, cert, card_id], |row| {
            Ok((row.get::<_, i32>(0)? != 0, row.get::<_, u32>(1)?))
        })?;
        Ok(rows.collect::<rusqlite::Result<_>>()?)
    }

    pub fn all_cards(&self, user_key: &str, cert: &str) -> Result<Vec<CardState>> {
        let mut stmt = self.0.prepare(
            "SELECT id, cert, stability, difficulty, due, last_review, reps
             FROM cards WHERE user_key = ?1 AND cert = ?2",
        )?;
        let rows = stmt.query_map(params![user_key, cert], CardState::from_row)?;
        Ok(rows.collect::<rusqlite::Result<_>>()?)
    }

    pub fn all_reviews(&self, user_key: &str, cert: &str) -> Result<Vec<(String, bool, u32)>> {
        let mut stmt = self
            .0
            .prepare("SELECT card_id, correct, rating FROM reviews WHERE user_key = ?1 AND cert = ?2")?;
        let rows = stmt.query_map(params![user_key, cert], |row| {
            Ok((
                row.get::<_, String>(0)?,
                row.get::<_, i32>(1)? != 0,
                row.get::<_, u32>(2)?,
    ...
    pub fn recent_sessions(&self, user_key: &str, cert: &str, n: usize) -> Result<Vec<(String, u32, u32)>> {
        let mut stmt = self.0.prepare(
            "SELECT date, total, correct FROM sessions
             WHERE user_key = ?1 AND cert = ?2 ORDER BY id DESC LIMIT ?3",
        )?;
        let rows = stmt.query_map(params![user_key, cert, n as u32], |row| {
            Ok((
                row.get::<_, String>(0)?,
                row.get::<_, u32>(1)?,
                row.get::<_, u32>(2)?,
...
fn migrate_schema(conn: &Connection) -> Result<()> {
    ...
    if has_reviews {
        if !has_column(conn, "reviews", "selected_letter")? {
            conn.execute_batch("ALTER TABLE reviews ADD COLUMN selected_letter TEXT")?;
        }
    }

    // Migration 3: add user_key to all user-scoped tables (default 'default' for existing data)
    if !has_column(conn, "cards", "user_key")? {
        conn.execute_batch(
            "
            ALTER TABLE cards RENAME TO cards_m3;
            CREATE TABLE cards (
                user_key    TEXT NOT NULL,
                id          TEXT NOT NULL,
                cert        TEXT NOT NULL,
            ...
            INSERT OR REPLACE INTO cards (user_key, id, cert, stability, difficulty, due, last_review, reps)
            SELECT 'default', id, cert, stability, difficulty, due, last_review, reps
            FROM cards_m3;
            DROP TABLE cards_m3;
            ",
        )?;
    ...
    if has_reviews && !has_column(conn, "reviews", "user_key")? {
        conn.execute_batch("ALTER TABLE reviews ADD COLUMN user_key TEXT NOT NULL DEFAULT 'default'")?;
    }
    ...
    if has_sessions && !has_column(conn, "sessions", "user_key")? {
        conn.execute_batch("ALTER TABLE sessions ADD COLUMN user_key TEXT NOT NULL DEFAULT 'default'")?;
    }

    let has_pending: bool = conn.query_row(
        "SELECT EXISTS(SELECT 1 FROM sqlite_master WHERE type='table' AND name='pending_sessions')",
        [],
        |row| row.get(0),
    )?;
    if has_pending && !has_column(conn, "pending_sessions", "user_key")? {
        conn.execute_batch(
            "
            ALTER TABLE pending_sessions RENAME TO pending_sessions_m3;
            ...
            INSERT INTO pending_sessions (user_key, cert, card_ids, control_mode, control_domain, started_at)
            SELECT 'default', cert, card_ids, control_mode, control_domain, started_at
            FROM pending_sessions_m3;
            DROP TABLE pending_sessions_m3;
            ",
        )?;
    }
...
mod tests {
    ...
    fn get_card_unknown_returns_default() {
        let db = new_db();
        let card = db.get_card(U, "cert-a", "nonexistent").unwrap();
    ...
    fn get_card_propagates_row_errors() {
        let db = new_db();
        db.0.execute(
            "INSERT INTO cards (user_key, id, cert, reps) VALUES (?1, ?2, ?3, ?4)",
            params!["user", "bad", "cert", -1],
        )
        .unwrap();

        assert!(db.get_card(U, "cert", "bad").is_err());
    }
    ...
    fn record_review_and_get_card_round_trip() {
        ...
        db.record_review(U, &card, "q1", "cert-a", true, 4, None)
            .unwrap();

        let retrieved = db.get_card(U, "cert-a", "q1").unwrap();
    ...
    fn record_review_stores_review_history() {
        ...
        db.record_review(U, &card, "q-rev", "cert-a", true, 4, None)
            .unwrap();
        db.record_review(U, &card, "q-rev", "cert-a", false, 1, None)
            .unwrap();

        let reviews = db.reviews_for(U, "cert-a", "q-rev").unwrap();
    ...
    fn reviews_for_unknown_card_is_empty() {
        let db = new_db();
        let reviews = db.reviews_for(U, "cert-a", "no-such-card").unwrap();
    ...
    fn insert_session_and_retrieve() {
        let db = new_db();
        db.insert_session(U, "cert-x", 10, 7).unwrap();

        let sessions = db.recent_sessions(U, "cert-x", 5).unwrap();
    ...
    fn recent_sessions_respects_limit() {
        let db = new_db();
        for i in 0..5u32 {
            db.insert_session(U, "cert-lim", 10, i).unwrap();
        }
        let sessions = db.recent_sessions(U, "cert-lim", 3).unwrap();
    ...
    fn all_cards_returns_only_matching_cert() {
        ...
        db.record_review(U, &a, "c1", "certA", true, 4, None).unwrap();
        db.record_review(U, &b, "c2", "certB", true, 4, None).unwrap();

        let cards_a = db.all_cards(U, "certA").unwrap();
        assert_eq!(cards_a.len(), 1);
        assert_eq!(cards_a[0].id, "c1");

        let cards_b = db.all_cards(U, "certB").unwrap();
    ...
    fn all_reviews_returns_only_matching_cert() {
        ...
        db.record_review(U, &card_a, "r1", "certA", true, 4, None)
            .unwrap();
        db.record_review(U, &card_b, "r2", "certB", false, 1, None)
            .unwrap();

        let revs_a = db.all_reviews(U, "certA").unwrap();
        assert_eq!(revs_a.len(), 1);
        assert_eq!(revs_a[0].0, "r1");
        assert!(revs_a[0].1);

        let revs_b = db.all_reviews(U, "certB").unwrap();
    ...
    fn record_review_updates_existing_card() {
        ...
        db.record_review(U, &v1, "upd", "c", true, 4, None).unwrap();
        ...
        db.record_review(U, &v2, "upd", "c", true, 4, None).unwrap();

        let retrieved = db.get_card(U, "c", "upd").unwrap();
        assert_eq!(retrieved.reps, 2);
        assert_eq!(retrieved.due, Some("2026-07-01".to_string()));
        assert_eq!(db.reviews_for(U, "c", "upd").unwrap().len(), 2);
    }
    ...
    fn duplicate_card_ids_are_scoped_by_cert() {
        ...
        db.record_review(U, &cert_a, "shared", "cert-a", true, 4, None)
            .unwrap();
        db.record_review(U, &cert_b, "shared", "cert-b", true, 4, None)
            .unwrap();

        let card_a = db.get_card(U, "cert-a", "shared").unwrap();
        let card_b = db.get_card(U, "cert-b", "shared").unwrap();
    ...
    fn reviews_for_are_scoped_by_cert() {
        ...
        db.record_review(U, &cert_a, "shared-review", "cert-a", true, 4, None)
            .unwrap();
        db.record_review(U, &cert_b, "shared-review", "cert-b", false, 1, None)
            .unwrap();

        assert_eq!(
            db.reviews_for(U, "cert-a", "shared-review").unwrap(),
            vec![(true, 4)]
        );
        assert_eq!(
            db.reviews_for(U, "cert-b", "shared-review").unwrap(),
            vec![(false, 1)]
        );
    }

    #[test]
    fn cards_are_scoped_by_user_key() {
        let db = new_db();
        let card = sample_card("q1", "cert-a");
        db.record_review("alice", &card, "q1", "cert-a", true, 4, None).unwrap();
        db.record_review("bob", &card, "q1", "cert-a", false, 1, None).unwrap();

        let alice = db.get_card("alice", "cert-a", "q1").unwrap();
        let bob = db.get_card("bob", "cert-a", "q1").unwrap();
        assert_eq!(alice.reps, 1);
        assert_eq!(bob.reps, 1);

        assert_eq!(db.all_cards("alice", "cert-a").unwrap().len(), 1);
        assert_eq!(db.all_reviews("alice", "cert-a").unwrap().len(), 1);
        assert_eq!(db.all_reviews("bob", "cert-a").unwrap().len(), 1);
    }
    ...
    fn open_at_migrates_legacy_card_primary_key() {
        ...
        db.record_review("default", &cert_b, "shared", "cert-b", true, 4, None)
            .unwrap();

        assert_eq!(db.get_card("default", "cert-a", "shared").unwrap().reps, 1);
        assert_eq!(db.get_card("default", "cert-b", "shared").unwrap().cert, "cert-b");
    }
    ...
    fn open_uses_default_home_path() {
        ...
        unsafe {
            std::env::remove_var("STUDY_ENGINE_DB_PATH");
            std::env::set_var("HOME", dir.path());
        }

        let db = Db::open_or_at(None).unwrap();
        db.insert_session("default", "cert-default", 1, 1).unwrap();
